...
const { GraphQLInt, GraphQLList, GraphQLEnumType, GraphQLBoolean, GraphQLObjectType, GraphQLString } = graphqlRoot
...
function any(postfix) {
  return (fields, modelClass) => reducePrimitiveFields(fields, modelClass, (args, field, propName, columnName) => {
    args[propName + postfix] = {
      type: field.type,
      query(query, value) {
        console.log({ field: fullCol(columnName, modelClass), value })
        query.where(fullCol(columnName, modelClass), '@>', `{${value}}`)
      }
    }
...
function distinct(argName) {
  return () => {
    ...
    args[argName] = {
      ...
      query(query, value) {
        query.distinct(`${query._modelClass.tableName}.${value}`)
      }
    }
