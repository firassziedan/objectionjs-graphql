...
const redisExists = process.env.REDIS_PORT && process.env.REDIS_HOST
let redisClient
if (redisExists) {
...
class SchemaBuilder {
  ...
  _resolverForModel(modelData, extraQuery) {
    return async(ctx, ignore1, httpRequest, data) => {
      let redisKey
      if (redisExists) {
        redisKey = 'gqlCache'.concat(JSON.stringify(httpRequest.body))
        const cachedResult = await new Promise((resolve, reject) => {
          redisClient.get(redisKey, (error, value) => {
      ...
      if (redisExists) {
        ...
        await redisClient.setex(redisKey, parseInt(process.env.GQL_CACHE_TIMEOUT || 3600), JSON.stringify(result))
        return result
      }
