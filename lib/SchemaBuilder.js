...
class SchemaBuilder {
  constructor(options = {}) {
    if (options.enableCache) {
      this.cache = new Cache(options, options.enableCache)
  ...
  async _checkCache(httpRequest) {
    if (this.cacheEnabled) {
      const cached = await this.cache.getCache(httpRequest)
      return (cached || null)
    }
  }
  ...
  _resolverForModel(modelData, extraQuery) {
    return async(ctx, ignore1, httpRequest, data) => {
      const cached = await this._checkCache(httpRequest)
      if (cached) {
        return cached
      }
      ctx = ctx || {}
      ...
      await this.cache.cacheResult(httpRequest, result)
      return result
    }
  }
